%--------------------------------------------------------------------------
% construct  K-neighbor graph  with random nodes and random connections
...
% 2. Connect two nodes by an edge if either of the paired nodes is one of 
%    the K closest neighbors of the other one.
...
for ik=1:length(kk)
    ...
    for i=1:n-1
        for j=i+1:n
            A0(i,j)=geodistkm([ylat(i),xlon(i)],[ylat(j),xlon(j)]);
            A0(j,i)=A0(i,j);
        end
    end
    for i=1:n
        [~,idx_ascend]=sort(A0(i,:),'ascend');
        for j=1:n        
            % if the node j is the k-closest neighbor of node i
            if sum(idx_ascend(1:k)==j)==1
                A(i,j)=A0(i,j);
                A(j,i)=A(i,j);
            end
        end
...
for ip=1:6
    ...
    p=sum(sum(A>0))/(n*(n-1));
    title(['f=',num2str(round(p,1)),', K=', num2str(k),', l_{',num2str(s),',',num2str(t),'}=',...
